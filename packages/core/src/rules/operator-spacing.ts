...
export const operatorSpacing: Rule<undefined, 'operator-spacing'> = {
  ...
  check: ({ tokens, comments, whitespaces, lines }) => {
    ...
    for (let i = 0; i < content.length; i++) {
      ...
      /* End of line — a wrapped expression, left to the indent rules. Before a `;` the gap is the terminator's. */
      const after = content[i + 1];

      if (after !== undefined && !ownsGapBefore(after) && !closesLine(whitespaces, lines, last)) {
