...
- **Never guess Qlik's syntax.** What a keyword means, which delimiters a name accepts, how a construct escapes, where a quoting rule stops applying — look it up in the [Qlik Sense help](https://help.qlik.com/en-US/sense/) and cite the page from the code or test that rests on it. This project models a language it cannot execute, so every unchecked assumption about Qlik is a bug waiting for somebody's script. Community threads are context, never evidence.
- **In doubt, leave the construct alone.** A rule that might break a script's syntax or change what it loads is worse than no rule at all. Where the semantics are unverified, skip the construct rather than transform it, and say in the rule reference that it is skipped and why. A missed cleanup costs a reader nothing; a corrupted load costs them a reload and their trust.
- **What the text _is_, the lexer decides.** Whether something is whitespace, a line break, a comment or a keyword is a claim about the language. A rule that answers it a second time — a character walk, a `[ \t]` pattern — drifts from the first answer, and that is how rules came to delete comments they could not see. Rules read the structure the engine hands them (`tokens`, `comments`, `whitespaces`, `lines`); reaching for the raw script is caught by ESLint and takes a written reason. Where the structure does not carry what a rule needs, ask whether the lexer should carry it before interpreting the text in the rule.
